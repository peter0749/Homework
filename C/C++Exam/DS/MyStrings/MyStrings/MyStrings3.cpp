...
public:
	...
	friend bool operator==(const char * t, const MyStrings & MyStr);
...
//--------------------------------------------------
bool operator==(const char * t, const MyStrings & MyStr)
{
	if(strcmp(t, MyStr.str)==0)
		return true;
	else
		return false;
};
...
//     if(fin.eof()==true)
 ...
 //    fin.getline((char *)&temp, sizeof(&temp));
		...
		else if((isColor==0)&&(name_toggle==1))
		{
			...
			char UpperA='A';
			usrname += temp;
			mystr = usrname.c_str() >> mystr;
		//	mystr = myName >> mystr;
		//	fout<<;
		//	fout <<"Please enter your name: "<< usrname.c_str() <<"\r\n";
			fout <<"Please enter your name: "<< mystr.ShowStr() <<"\r\n";
	        MyStrings tempstr = (myName >> mystr)+".";	//My name is + Maria Pan	
	
			
		//	mystr << usrname.c_str();
			cout << tempstr.ShowStr() << endl; //My name is Maria Pan.
			fout << tempstr.ShowStr() << "\r\n";
			mystr << studentStr;
			mystr = myName >> mystr;
