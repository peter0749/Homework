...
int  auto_game(int tar, int arr[][MAX])     //Recursively find the best solution?
{
    int state;
    int fakeptr = -1;
    int cotar = (tar==1?2:1);
    ...
    while(1)
    {
        ...
        //system("pause");
        compute(cotar,bk,1,0);
        if(is_tie(bk))
        {
            return -1;
        }

        state = is_n(cotar,bk,5);
...
void compute(int tar, int arr[][MAX], int difcult, int recu)      //difficulty 0:easy 1:hard
{
    int remain[500][2];
    //int bk[MAX][MAX];
    int i,j,k = 0;
    ...
    if(difcult)             //To adjust difficulty
    {
        ...
        for(i=4; i>=1; i--)
        {
            if(automode(tar,tar,arr,i,1,recu))
            {
                return;   //Attack
            }
        }
        ...
        for(i=4; i>=1; i--)
        {
            if(automode(cotar,tar,arr,i,1,recu))
            {
                //printf("i= %d\n",i);
                return;   //Defend
            }
    ...
    if(automode(tar,tar,arr,4,0,0))
    {
        return;   //Attack
    }
    if(automode(cotar,tar,arr,4,0,0))
    {
        //printf("i= %d\n",i);
        return;   //Defend
    }
    ...
    for(i=3; i>=2; i--)
    {
        if(automode(cotar,tar,arr,i,0,0))
        {
            //printf("i= %d\n",i);
            return;   //Defend
        }
    }
    if(automode(tar,tar,arr,1,0,0))
    {
        return;   //Attack
    }

    if(automode(cotar,tar,arr,1,0,0))
    {
        //printf("i= %d\n",i);
        return;   //Defend
    }
...
int automode(int tar,int cotar,int arr[][MAX], int n, int diflty,int recu)       //diflty 0:easy 1:hard
{
    int i, j, dir, k =0;
    int shifter;       //Shifter
    int bk[MAX][MAX];
    ...
    if(k>0)
    {
        ...
        if(diflty==0)
        {
            ...
            for(i=0; i<k; i++)
            {
                ...
                if(recu==1 && auto_game(tar,bk)==1 )     //Recursively find better solution?
                {
                    c = lc;
                    arr[valid[i][0]][valid[i][1]] = cotar;
                    a_cont(valid[i][0], valid[i][1]);
